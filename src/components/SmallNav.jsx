        ...
        <div className="basis-1/2">
            ...
            <div className="basis-1/2 flex justify-end">
                    ...
                    <div className="flex text-2xl">           
                        ...
                        <input  value={s} onChange={(e)=>setS(e.target.value)} type="text" placeholder="Search Here" className="rounded-3xl block p-2 text-md backdrop-blur-md bg-white/5 focus:outline-none" />
                            <div className={`${s !== "" ? "block" : "hidden"} bodys  absolute top-full p-2 rounded-2xl overflow-y-scroll bg-white text-black h-96 w-full`}>
                                {
                                    sData.map((value,i)=>{
                                        return <div key={i} className="flex cursor-pointer overflow-hidden rounded-2xl   font-Poppins my-1 p-2 bg-slate-200">
                                            <div className="basis-2/5"><img className="w-full h-24 rounded-2xl " src={value.img} alt="Product" /></div>
                                            <div className="basis-3/5 p-2 box-border">
                                            <p>{value.title}</p>
